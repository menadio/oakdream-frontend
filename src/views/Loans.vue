<template>
  <div class="mt-8">
      <!-- page title -->
    <div class="mb-3">
      <h1 class="text-left font-sans font-semibold">Loans</h1>
    </div>

    <!-- statistics cards -->
    <div class="bg-white rounded h-auto shadow-lg flex content-center flex-wrap text-left mb-8">
      <div class="w-1/3 border-r md:py-5 md:px-5 font-sans">
        <p class="text-2xl font-semibold text-indigo-800">{{ stats.loansTotal }}</p>
        <p class="text-gray-500 font-semibold text-sm">Total Loans</p>
      </div>

      <div class="w-1/3 border-r py-5 px-5 font-sans">
        <p class="text-2xl font-semibold text-indigo-800">{{ stats.loansApproved }}</p>
        <p class="text-gray-500 font-semibold text-sm">Approved Loans</p>
      </div>

      <div class="w-1/3 py-5 px-5 font-sans">
        <p class="text-2xl font-semibold text-indigo-800">{{ stats.loansDenied }}</p>
      ...
      <table class="table-auto w-full font-sans">
        ...
        <tbody class="border text-gray-600">
          <tr v-for="loan in loans" v-bind:key="loan.id" class="border">
            <td class="px-3 py-3 text-left">{{ loan.customer }}</td>
            ...
            <td
              ...
              v-bind:class="{'text-red-600':loan.status == 'denied', 'text-green-600':loan.status == 'approved'}"
            >{{ loan.status }}</td>
            <td class="px-3 py-3">
              <svg
